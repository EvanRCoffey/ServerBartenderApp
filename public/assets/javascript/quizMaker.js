...
function getCorrectAnswerNumbers(questionObj) {
    for (var i = 0; i < questionObj.correctAnswers.length; i++) {
        if (questionObj.correctAnswers[i] === questionObj.answer1) { questionObj.correctAnswers[i] = 1 } else if (questionObj.correctAnswers[i] === questionObj.answer2) { questionObj.correctAnswers[i] = 2 } else if (questionObj.correctAnswers[i] === questionObj.answer3) { questionObj.correctAnswers[i] = 3 } else if (questionObj.correctAnswers[i] === questionObj.answer4) { questionObj.correctAnswers[i] = 4 }
    }
    return questionObj;
}
    ...
    if (quizSize === 30) {
        ...
        if (allQuizQuestions.q5Array.length > 0) {
            ...
            for (var i = 0; i < allQuizQuestions.q5Array.length; i++) {
                if (counter < 4) {
                    var Question = {
                        question: allQuizQuestions.q5Array[i].question,
                        answer1: allQuizQuestions.q5Array[i].fourAnswersArray[0],
                        answer2: allQuizQuestions.q5Array[i].fourAnswersArray[1],
                        answer3: allQuizQuestions.q5Array[i].fourAnswersArray[2],
                        answer4: allQuizQuestions.q5Array[i].fourAnswersArray[3],
                        correctAnswers: allQuizQuestions.q5Array[i].correctAnswersIndexesArray
                    }
                    Question = getCorrectAnswerNumbers(Question);
                    randomizedQuiz.push(Question);
                    counter++;
...
function fillQuizQuestionsArray(questionsNeeded, randomizedQuiz) {
    ...
    for (var i = 0; i < questionsNeeded; i++) {
        if (allQuizQuestions.q3DesArray.length >= 1 || allQuizQuestions.q3EntArray.length >= 1 || allQuizQuestions.q3AppArray.length >= 1 && flag1) {
            var tempArray = [];

            var limit = allQuizQuestions.q3DesArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q3DesArray[j])) {
                    tempArray.push(allQuizQuestions.q3DesArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q3EntArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q3EntArray[j])) {
                    tempArray.push(allQuizQuestions.q3EntArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q3AppArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q3AppArray[j])) {
                    tempArray.push(allQuizQuestions.q3AppArray[j]);
                    j = limit;
                }
            }
        ...
        } else if (allQuizQuestions.q4DesArray.length >= 1 || allQuizQuestions.q4EntArray.length >= 1 || allQuizQuestions.q4AppArray.length >= 1 && flag2) {
            var tempArray = [];

            var limit = allQuizQuestions.q4DesArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q4DesArray[j])) {
                    tempArray.push(allQuizQuestions.q4DesArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q4EntArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q4EntArray[j])) {
                    tempArray.push(allQuizQuestions.q4EntArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q4AppArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q4AppArray[j])) {
                    tempArray.push(allQuizQuestions.q4AppArray[j]);
                    j = limit;
                }
            }
        ...
        } else if (allQuizQuestions.q1AppArray.length >= 1 || allQuizQuestions.q1EntArray.length >= 1 && flag3) {
            var tempArray = [];

            var limit = allQuizQuestions.q1EntArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q1EntArray[j])) {
                    tempArray.push(allQuizQuestions.q1EntArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q1AppArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q1AppArray[j])) {
                    tempArray.push(allQuizQuestions.q1AppArray[j]);
                    j = limit;
                }
            }
        ...
        } else if (allQuizQuestions.q2AppArray.length === 1 || allQuizQuestions.q2EntArray.length === 1 && flag4) {
            var tempArray = [];

            var limit = allQuizQuestions.q2EntArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q2EntArray[j])) {
                    tempArray.push(allQuizQuestions.q2EntArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q2AppArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q2AppArray[j])) {
                    tempArray.push(allQuizQuestions.q2AppArray[j]);
                    j = limit;
                }
            }
        ...
        } else if (allQuizQuestions.q6DesArray.length === 1 || allQuizQuestions.q6EntArray.length === 1 || allQuizQuestions.q6AppArray.length === 1 && flag5) {
            var tempArray = [];

            var limit = allQuizQuestions.q6DesArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q6DesArray[j])) {
                    tempArray.push(allQuizQuestions.q6DesArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q6EntArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q6EntArray[j])) {
                    tempArray.push(allQuizQuestions.q6EntArray[j]);
                    j = limit;
                }
            }

            var limit = allQuizQuestions.q6AppArray.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q6AppArray[j])) {
                    tempArray.push(allQuizQuestions.q6AppArray[j]);
                    j = limit;
                }
            }
            ...
            var limit = allQuizQuestions.q5Array.length;
            for (var j = 0; j < limit; j++) {
                if (!randomizedQuiz.includes(allQuizQuestions.q5Array[j])) {
                    randomizedQuiz.push(allQuizQuestions.q5Array[j]);
                    j = limit;
                }
            }
        }
    ...
    $.post("/checkMenuJSON", idObj).then(function(data2) {
        ...
        for (var j = 0; j < uniqueAppDescr.length; j++) {
            ...
            for (var i = 0; i < allApp.length; i++) {
                if (randomThing === "aLaCarteOrBiteSize") { if (allApp[i].descriptors.aLaCarteOrBiteSize) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "cheesy") { if (allApp[i].descriptors.cheesy) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "fresh") { if (allApp[i].descriptors.fresh) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "fried") { if (allApp[i].descriptors.fried) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "healthy") { if (allApp[i].descriptors.healthy) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "hearty") { if (allApp[i].descriptors.hearty) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "indulgent") { if (allApp[i].descriptors.indulgent) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "light") { if (allApp[i].descriptors.light) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "plain") { if (allApp[i].descriptors.plain) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "popular") { if (allApp[i].descriptors.popular) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "raw") { if (allApp[i].descriptors.raw) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "salty") { if (allApp[i].descriptors.salty) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "servedCold") { if (allApp[i].descriptors.servedCold) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "spicy") { if (allApp[i].descriptors.spicy) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "sweet") { if (allApp[i].descriptors.sweet) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "toShare") { if (allApp[i].descriptors.toShare) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "vegetarian") { if (allApp[i].descriptors.vegetarian) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } }
            }
        ...
        for (var j = 0; j < uniqueEntDescr.length; j++) {
            ...
            for (var i = 0; i < allEnt.length; i++) {
                if (randomThing === "aLaCarteOrBiteSize") { if (allEnt[i].descriptors.aLaCarteOrBiteSize) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "cheesy") { if (allEnt[i].descriptors.cheesy) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "fresh") { if (allEnt[i].descriptors.fresh) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "fried") { if (allEnt[i].descriptors.fried) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "healthy") { if (allEnt[i].descriptors.healthy) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "hearty") { if (allEnt[i].descriptors.hearty) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "indulgent") { if (allEnt[i].descriptors.indulgent) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "light") { if (allEnt[i].descriptors.light) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "plain") { if (allEnt[i].descriptors.plain) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "popular") { if (allEnt[i].descriptors.popular) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "raw") { if (allEnt[i].descriptors.raw) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "salty") { if (allEnt[i].descriptors.salty) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "servedCold") { if (allEnt[i].descriptors.servedCold) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "spicy") { if (allEnt[i].descriptors.spicy) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "sweet") { if (allEnt[i].descriptors.sweet) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "toShare") { if (allEnt[i].descriptors.toShare) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "vegetarian") { if (allEnt[i].descriptors.vegetarian) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        for (var j = 0; j < uniqueDesDescr.length; j++) {
            ...
            for (var i = 0; i < allDes.length; i++) {
                if (randomThing === "chocolatey") { if (allDes[i].descriptors.chocolatey) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "fruity") { if (allDes[i].descriptors.fruity) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "healthy") { if (allDes[i].descriptors.healthy) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "light") { if (allDes[i].descriptors.light) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "popular") { if (allDes[i].descriptors.popular) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "rich") { if (allDes[i].descriptors.rich) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "tart") { if (allDes[i].descriptors.tart) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "toShare") { if (allDes[i].descriptors.toShare) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        for (var j = 0; j < uniqueAppIngr.length; j++) {
            ...
            var noThing = [];
            for (var i = 0; i < allApp.length; i++) {
                if (randomThing === "beef") { if (allApp[i].ingredients.beef) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "chicken") { if (allApp[i].ingredients.chicken) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "lamb") { if (allApp[i].ingredients.lamb) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "otherProtein") { if (allApp[i].ingredients.otherProtein) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "pork") { if (allApp[i].ingredients.pork) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "seafood") { if (allApp[i].ingredients.seafood) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        for (var j = 0; j < uniqueEntIngr.length; j++) {
            ...
            var noThing = [];
            for (var i = 0; i < allEnt.length; i++) {
                if (randomThing === "beef") { if (allEnt[i].ingredients.beef) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "chicken") { if (allEnt[i].ingredients.chicken) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "lamb") { if (allEnt[i].ingredients.lamb) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "otherProtein") { if (allEnt[i].ingredients.otherProtein) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "pork") { if (allEnt[i].ingredients.pork) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "seafood") { if (allEnt[i].ingredients.seafood) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        for (var j = 0; j < uniqueDesIngr.length; j++) {
            ...
            var noThing = [];
            for (var i = 0; i < allDes.length; i++) {
                if (randomThing === "cake") { if (allDes[i].ingredients.cake) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "pie") { if (allDes[i].ingredients.pie) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "iceCream") { if (allDes[i].ingredients.iceCream) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        for (var j = 0; j < uniqueAllVio.length; j++) {
            ...
            var noThing = [];
            for (var i = 0; i < allApp.length; i++) {
                if (randomThing === "eggs") { if (allApp[i].allergyViolations.eggs) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "fish") { if (allApp[i].allergyViolations.fish) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "gluten") { if (allApp[i].allergyViolations.gluten) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "milk") { if (allApp[i].allergyViolations.milk) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "peanuts") { if (allApp[i].allergyViolations.peanuts) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "shellfish") { if (allApp[i].allergyViolations.shellfish) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "soy") { if (allApp[i].allergyViolations.soy) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "treeNuts") { if (allApp[i].allergyViolations.treeNuts) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } } else if (randomThing === "wheat") { if (allApp[i].allergyViolations.wheat) { hasThing.push(allApp[i].name); } else { noThing.push(allApp[i].name); } }
            }
            for (var i = 0; i < allEnt.length; i++) {
                if (randomThing === "eggs") { if (allEnt[i].allergyViolations.eggs) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "fish") { if (allEnt[i].allergyViolations.fish) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "gluten") { if (allEnt[i].allergyViolations.gluten) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "milk") { if (allEnt[i].allergyViolations.milk) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "peanuts") { if (allEnt[i].allergyViolations.peanuts) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "shellfish") { if (allEnt[i].allergyViolations.shellfish) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "soy") { if (allEnt[i].allergyViolations.soy) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "treeNuts") { if (allEnt[i].allergyViolations.treeNuts) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } } else if (randomThing === "wheat") { if (allEnt[i].allergyViolations.wheat) { hasThing.push(allEnt[i].name); } else { noThing.push(allEnt[i].name); } }
            }
            for (var i = 0; i < allDes.length; i++) {
                if (randomThing === "eggs") { if (allDes[i].allergyViolations.eggs) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "fish") { if (allDes[i].allergyViolations.fish) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "gluten") { if (allDes[i].allergyViolations.gluten) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "milk") { if (allDes[i].allergyViolations.milk) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "peanuts") { if (allDes[i].allergyViolations.peanuts) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "shellfish") { if (allDes[i].allergyViolations.shellfish) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "soy") { if (allDes[i].allergyViolations.soy) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "treeNuts") { if (allDes[i].allergyViolations.treeNuts) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } } else if (randomThing === "wheat") { if (allDes[i].allergyViolations.wheat) { hasThing.push(allDes[i].name); } else { noThing.push(allDes[i].name); } }
            }
            if (hasThing.length >= 3) {
                for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
                sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allApp.length; i++) {
            if (allApp[i].descriptors.popular) { hasThing.push(allApp[i].name) } else { noThing.push(allApp[i].name) }
        }
        if (hasThing.length >= 3) {
            for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
            sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allEnt.length; i++) {
            if (allEnt[i].descriptors.popular) { hasThing.push(allEnt[i].name) } else { noThing.push(allEnt[i].name) }
        }
        if (hasThing.length >= 3) {
            for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
            sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allDes.length; i++) {
            if (allDes[i].descriptors.popular) { hasThing.push(allDes[i].name) } else { noThing.push(allDes[i].name) }
        }
        if (hasThing.length >= 3) {
            for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
            sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allApp.length; i++) {
            if (allApp[i].descriptors.vegetarian) { hasThing.push(allApp[i].name) } else { noThing.push(allApp[i].name) }
        }
        if (hasThing.length >= 3) {
            for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
            sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allEnt.length; i++) {
            if (allEnt[i].descriptors.vegetarian) { hasThing.push(allEnt[i].name) } else { noThing.push(allEnt[i].name) }
        }
        if (hasThing.length >= 3) {
            for (var i = 0; i < 3; i++) { sampleFourAnswers.push(returnCorrectName(hasThing, sampleFourAnswers)); }
            sampleFourAnswers.push(returnIncorrectName(noThing, sampleFourAnswers));
        ...
        var noThing = [];
        for (var i = 0; i < allEnt.length; i++) {
            if (allEnt[i].secret) { hasThing.push(allEnt[i].name) } else { noThing.push(allEnt[i].name) }
        }
        for (var i = 0; i < allApp.length; i++) {
            if (allApp[i].secret) { hasThing.push(allApp[i].name) } else { noThing.push(allApp[i].name) }
        }
        for (var i = 0; i < allDes.length; i++) {
            if (allDes[i].secret) { hasThing.push(allDes[i].name) } else { noThing.push(allDes[i].name) }
        }
        ...
        shuffle(allQuizQuestions.q1AppArray);
        shuffle(allQuizQuestions.q1AppArray);
        shuffle(allQuizQuestions.q1EntArray);
        shuffle(allQuizQuestions.q1EntArray);
        shuffle(allQuizQuestions.q3AppArray);
        shuffle(allQuizQuestions.q3AppArray);
        shuffle(allQuizQuestions.q3EntArray);
        shuffle(allQuizQuestions.q3EntArray);
        shuffle(allQuizQuestions.q4AppArray);
        shuffle(allQuizQuestions.q4AppArray);
        shuffle(allQuizQuestions.q4EntArray);
        shuffle(allQuizQuestions.q4EntArray);
        shuffle(allQuizQuestions.q4DesArray);
        shuffle(allQuizQuestions.q4DesArray);
        shuffle(allQuizQuestions.q5Array);
        shuffle(allQuizQuestions.q5Array);
